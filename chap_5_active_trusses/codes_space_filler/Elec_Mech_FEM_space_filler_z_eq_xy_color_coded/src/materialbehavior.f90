 ...
 !!    along with this program.  If ot, see <http://www.gnu.org/licenses/>.
...
!     =========================geometrical variables
      ...
      real(iwp)::x1,x2,x3,r,kappa,epsilon
real(iwp)::strain(3,3);
real(iwp)::sigma_shape_desired(3,3);
real(iwp)::electric_feild(3);
real(iwp)::volumetric_stress
...
!!!Edit this part if a constitutive equation is used
each_truss_strain(noelem)=desired_normal_strain_in_truss
applied_normal_strain_in_truss=desired_normal_strain_in_truss

sigma_shape_desired=0.0d0
do i=1,dimen;
do j=1,dimen;
sigma_shape_desired(i,j)=applied_normal_strain_in_truss*&
element_direction_normal(i)*element_direction_normal(j)
enddo;enddo
