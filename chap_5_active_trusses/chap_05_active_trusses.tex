...
Shape changing in these truss systems is controlled by an actuation of each truss component using a piezoelectric actuation. 
\\
...
\end{equation}
In order to induce the desired strain, each truss member associated with shape change, an electric field is prescribed to the truss member.
...
\end{equation}
where $E_{11}$ is the axial strain due to longitudinal axis of each truss member, $f_1(S_{11})$ is the strain due to the mechanical stress along the longitudinal axis of the truss $S_11$ and $f_2$ is the strain due to the electric field input, prescribed through the thickness of the truss, $E_3$.
...
The pin joints allow the members to rotate with respect to each others.
Each truss element is specified by two joints in the space and the shortest distance connect these two joints.
...
In the case of linear interpolation of variables and also geometry, the master shape function of the isoparametric element $\zeta \in [-1,1]$ is introduced as:
\begin{equation}
\begin{aligned}
...
If the truss system is intended to have a shape as is shown in Figure \ref{fig:xy_plane_desired_shape} the mapping between the reference and current configurations for $X_3=X_1 X_2$ surface is defined as following.
...
The length of all truss members is not equal in this truss configuration. 
...
Using tetrahedral as a truss configuration we can arrange the truss elements to take a beam like form.
...
Prescribing the above calculated strain field to the truss structure, the desired shape is attained.
...
\begin{equation}
\begin{aligned}
e_{truss} &=\sum_{i=1}^3 \sum_{j=1}^3 E_{ij}N^{truss}_i N^{truss}_j\\
E_{11} &=E_3 d_{311} 
\end{aligned}
...
$e_{truss}$ is the electric field that should be applied to each truss element to induce the deformation.
